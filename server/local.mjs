...
const types={'.html':'text/html; charset=utf-8','.css':'text/css; charset=utf-8','.js':'text/javascript; charset=utf-8','.json':'application/json; charset=utf-8','.jpg':'image/jpeg','.png':'image/png','.svg':'image/svg+xml','.mp4':'video/mp4','.mov':'video/quicktime','.webm':'video/webm','.wav':'audio/wav','.mp3':'audio/mpeg'};
...
async function readJSON(req){try{return JSON.parse((await readBody(req)).toString('utf8'));}catch{throw new Error('请求内容格式不正确');}}
    ...
    try{
      ...
      if(url.pathname==='/api/upload'&&req.method==='POST'){
        ...
        try{
          ...
          json(res,await startWorkflow(p,req.headers['x-voice']));
...
if(process.argv[1]&&import.meta.url===pathToFileURL(path.resolve(process.argv[1])).href){
  ...
  const stop=async()=>{server.close();await stopWorkflows();process.exit(0);};process.on('SIGINT',stop);process.on('SIGTERM',stop);
}
