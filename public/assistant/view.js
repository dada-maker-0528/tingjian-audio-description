...
import {createSession,openDraft,isAcceptedResult,makeContext,editDraft,valueFor,differences,undoCategory,compileDraft,applyInterpretation,beginRun,finishRun,failRun,acceptCandidate,discardCandidates} from './model.js';
import {assistantServices} from './services.js';
import {VoiceInput,recognitionConstructor} from './voice-input.js';
import {capturePanel,animatePanel} from '../ui-motion.js';
...
const sceneLabel=(film,id)=>{const s=film.scenes.find(s=>s.id===id);return s?`场景 ${s.storyNumber||film.scenes.indexOf(s)+1} · ${s.title}`:id;};
export class NarrationAssistant{
 constructor(dialog,hooks){this.el=dialog;this.hooks=hooks;this.mode='ordinary';this.category=null;this.view=null;this.parseId=0;this.busy=false;this.voiceMessage='';this.composing=false;
  ...
  dialog.addEventListener('focusin',e=>{this.touched=true;if(e.target.id==='assistant-input')this.hooks.pauseForInput?.();if(e.target.id==='assistant-input'||e.target.closest('[data-assistant="voice"]'))this.voice.prepare({connect:true});});
  dialog.addEventListener('pointerover',e=>{if(e.target.closest('[data-assistant="voice"]'))this.voice.prepare({connect:true});});
 ...
 open({film,taskId,settings,sceneId,playhead,phase,canContinue,quiet=false}){
  ...
  this.useScene(sceneId,playhead);if(this.plan.valid)this.view='card';this.el.hidden=false;this.el.classList.add('assistant-docked');this.render();this.voice.prepare();
 ...
 useScene(id,playhead){this.cancelVoice();this.parseId++;this.context=makeContext(this.origin.film,id,playhead??this.origin.film.scenes.find(s=>s.id===id).start,this.origin.taskId);this.hooks.services?.syncMediaState?.(this);this.draft=openDraft(this.session,this.context);this.person='';this.hooks.services?.syncS1VoiceDraft?.(this);
  const initialInput=this.hooks.services?.defaultInput?.(this);
  if(initialInput&&!this.draft.defaultInputApplied){
   this.draft.defaultInputApplied=true;
   if(this.draft.status==='editing'&&!this.draft.input&&!this.draft.revision&&!this.draft.history.length&&!this.session.candidates[id])this.draft.input=initialInput;
  }
  this.hooks.save();}
 syncPlayhead(sceneId,at){
  const completed=isAcceptedResult(this.session,this.draft);
  if(!this.visible||this.busy||this.voice.active||this.el.contains(document.activeElement)||this.draft?.input?.trim()||(!completed&&(this.pinnedScene||this.draft?.revision||this.draft?.status!=='editing'||this.session.candidates[this.context.sceneId])))return;
  if(sceneId!==this.context.sceneId){if(completed)this.pinnedScene=false;this.useScene(sceneId,at);this.category=null;this.view=this.plan.valid?'card':null;this.render();}else this.context.playhead=at;
 }
 hide(){this.cancelVoice();this.voice.discardPrepared();this.parseId++;this.el.hidden=true;if(this.el.parentNode!==document.body)document.body.append(this.el);}
 ...
 get plan(){const plan=compileDraft(this.session,this.draft,this.context);return this.hooks.services?.plan?.(plan,this)||plan;}
 get videoVersions(){return Boolean(this.hooks.services?.isVideo?.(this));}
 confirmationText(plan=this.plan){return spokenChanges(plan,{characters:this.context.registry,real:!!this.hooks.services,video:this.videoVersions})+(this.videoVersions&&this.draft.message?'\n'+this.draft.message:'');}
 ...
 notify(message,event='status'){
  ...
  if(!['ready','result'].includes(event)&&!(event==='error'&&this.draft?.status==='failed'))this.hooks.announce(message);this.message=message;const live=this.el.querySelector('#assistant-live');if(live)live.textContent=spoken;
 ...
 cardHTML(plan){
  ...
  const changes=plan.changes.map((p,i)=>{const field=FIELD_MAP[p.field],label=field.label+(p.targetCharacterId?' · '+(this.context.registry.find(c=>c.id===p.targetCharacterId)?.name||p.targetCharacterId):'');
   const editable=field.options.length&&field.type!=='set'&&field.type!=='alias';
   return `<li>${editable?`<label for="assistant-change-${i}">${esc(label)}</label>`:`<span class="assistant-change-label">${esc(label)}</span>`}<div class="assistant-change-value"><span class="assistant-previous">原设置 · ${esc(formatValue(p.field,p.before))}</span>${editable?`<select id="assistant-change-${i}" data-field="${esc(p.field)}" data-character="${esc(p.targetCharacterId||'')}" ${this.busy?'disabled':''}>${field.options.filter(o=>!(p.field==='naming_mode'&&!p.targetCharacterId&&o.value==='用户别名')).map(o=>`<option value="${esc(JSON.stringify(o.value))}" ${JSON.stringify(o.value)===JSON.stringify(p.value)?'selected':''}>${esc(o.label)}</option>`).join('')}</select>`:`<strong class="assistant-new-value">${esc(formatValue(p.field,p.value))}</strong>`}</div></li>`;
  }).join('');
  return `<div class="assistant-review"><article class="assistant-command" aria-label="本次修改指令"><div class="assistant-card-title"><h3 id="assistant-command-title" tabindex="-1">本次修改 <span>· ${plan.changes.length} 项</span></h3><span class="assistant-badge">${plan.valid?'待确认':'待补充'}</span></div>${plan.valid?`<p class="assistant-confirmation">${esc(spokenChanges(plan,{characters:this.context.registry,real:!!this.hooks.services,video:this.videoVersions}))}</p>`:''}${this.videoVersions&&d.message?`<details class="assistant-additional-note"><summary>补充说明</summary><p class="assistant-note">${esc(d.message)}</p></details>`:''}${button('speak',toolIcon('speaker')+'听修改指令','','assistant-listen')}<p class="assistant-command-target">${plan.targets.map(t=>esc(sceneLabel(this.origin.film,t.sceneId))).join('、')}</p></article><section class="assistant-settings" aria-label="修改设置"><h3>修改设置</h3><ul class="assistant-changes">${changes}</ul>${this.scopeHTML()}${plan.errors.length?`<p class="assistant-notice">${esc(plan.errors.join(' '))}</p>`:''}${plan.recovery?button('available-version','查看可试听的修改方案','','block'):''}</section></div><div class="assistant-plan-tools"><details class="assistant-execution-details"><summary>执行与保留内容</summary><dl><dt>执行内容</dt><dd>${esc(plan.execution||'等待有效修改')}</dd><dt>保持不变</dt><dd>未修改的标签、原片画面和原声。</dd><dt>应用范围</dt><dd>${esc(SCOPE_LABELS[d.scope])}${d.scope==='current_and_following'?'；先确认本场景，满意后再沿用。':'；不更改其他场景。'}</dd></dl></details><div class="assistant-card-actions">${button('prompt','查看完整提示词','id="assistant-show-prompt"','assistant-prompt-link')}</div><p class="assistant-meta">${this.videoVersions?(plan.mediaUnchanged?'确认后准备当前视频供试听，修改意见保留。':'确认后准备对应视频版本，原版保留。'):this.hooks.services?'本次生成真实配音，确认前不会执行。':'本次生成模拟结果，确认前不会执行。'}</p></div>`;
 }
 examplesHTML(){return `<div class="assistant-examples" aria-label="修改要求示例">${(this.hooks.services?.examples?.(this)||['旁白慢一点','动作讲清楚','少讲环境']).map(text=>button('example',text,`data-text="${text}"`)).join('')}</div>`;}
 ordinaryHTML(plan){
  const d=this.draft,history=d.history||[],hasChanges=plan.changes.length>0;
  return `${!hasChanges&&!d.clarification?`<div class="assistant-welcome"><h3>想怎样调整这段旁白？</h3><p>说出你的想法，检查修改方案后再应用。</p></div>`:''}${history.length?`<details class="assistant-history"><summary>之前的对话 · ${history.length} 条</summary>${history.map(h=>`<div class="assistant-message ${h.who==='user'?'is-user':''}"><strong>${h.who==='user'?'你':'旁白助手'}</strong><p>${esc(h.text)}</p>${h.card?'<span class="assistant-meta">历史记录 · 已被当前指令替代</span>':''}</div>`).join('')}</details>`:''}${d.clarification?`<section class="assistant-notice" aria-label="需要补充的信息"><strong>先确认一下</strong><p>${esc(d.clarification.message)}</p>${d.clarification.choices?.map(c=>button('clarify',esc(c.name),`data-person="${esc(c.id)}"`)).join('')||''}${plan.recovery&&!hasChanges?button('available-version','查看可试听的修改方案','','block'):''}</section>`:''}${hasChanges?this.cardHTML(plan):''}${d.message&&!hasChanges?`<p class="assistant-note">${esc(d.message)}</p>`:''}`;
 }
 ...
 resultHTML(){const result=this.draft.result,items=result?.candidates||[],unchanged=items.length>0&&items.every(c=>c.mediaUnchanged);return `<section class="assistant-result"><span class="assistant-badge">${unchanged?'修改方案':result.mediaKind==='video'?'新版视频':result.mediaKind==='audio'?'新版配音':'模拟结果'}</span><h3 tabindex="-1" id="assistant-result-title">${unchanged?'修改方案已就绪':result.mediaKind==='video'?'新版本已就绪':result.mediaKind==='audio'?'新配音已就绪':'修改方案已就绪'}</h3><p class="assistant-result-guidance">${esc(this.resultText())}</p>${items.map(c=>{const pending=this.session.candidates[c.sceneId]?.id===c.id,accepted=this.session.accepted[c.sceneId]?.id===c.id;return `<article class="assistant-result-item"><h4>${esc(sceneLabel(this.origin.film,c.sceneId))}</h4><span class="assistant-meta">${accepted?'已确认方案':pending?'待你检查':'已被更新的版本替代'}</span><ul>${c.changes.map(p=>`<li>${esc(FIELD_MAP[p.field].label)}：${esc(formatValue(p.field,p.before))} → <strong>${esc(formatValue(p.field,p.value))}</strong></li>`).join('')}</ul><details><summary>对比原版文案与参数</summary><p>同一原片范围：${c.start}—${c.end} 秒。</p>${c.candidateText.length?c.candidateText.map(n=>`<p><strong>${esc(n.status)}</strong><br>${esc(n.text)}</p>`).join(''):'<p>本场景暂无已标注旁白，不能编造文案或试听音轨。</p>'}<p>${c.mediaUnchanged?'当前视频已读取，可以试听；修改意见已记录。':c.mediaKind==='video'?'对应视频已读取，可以试听新旧版本。':c.audioGenerated&&c.timingVerified?'新配音已生成，已通过时长和原声窗口检查。':'未测量音频时长，未完成同步验证。'}</p></details>${c.narrationUrl||c.videoUrl?button('audition',c.mediaUnchanged?'试听当前视频':'试听新版',`data-scene="${esc(c.sceneId)}"`):''}${pending&&items.length>1?button('accept-one','确认这个场景的方案',`data-scene="${esc(c.sceneId)}"`):''}</article>`;}).join('')}<div class="assistant-card-actions">${button('review-original','查看原片')}${button('continue-edit','继续修改')}${button('discard',unchanged?'放弃本次修改':'放弃新版')}</div></section>`;}
 ...
 footerHTML(plan,result,allAccepted){
  ...
  if(result){const unchanged=d.result.candidates.length>0&&d.result.candidates.every(c=>c.mediaUnchanged);return `${button('speak-result','听结果摘要')}${allAccepted?button('continue-flow',this.origin.canContinue?'满意并继续':'满意并完成','','primary block'):button('accept-all',d.result.candidates.length===1?(this.origin.canContinue?'满意并继续':'满意并完成'):'请逐个确认场景方案',d.result.candidates.length===1?'':'disabled','primary block')}<p class="assistant-meta">${unchanged?'修改意见已记录，当前仍播放原有视频。':d.result.mediaKind==='video'?'采用后播放新版视频，原版保留。':d.result.mediaKind==='audio'?'采用后更新旁白，原版保留。':'确认的是模拟方案；原有配音保留。'}</p>`;}
  if(this.category)return `${button('show-card','查看修改指令','','primary block')}${button('natural','用自然语言补充','','block')}`;
  if(this.view==='card')return `<div class="assistant-confirm-actions"><p class="assistant-confirm-note">${plan.valid?(this.videoVersions?(plan.mediaUnchanged?'确认后准备当前视频供试听':'确认后准备新版视频'):this.hooks.services?'确认后生成新配音':'确认后预览模拟方案'):'请先补充修改要求'}<span>原片与原声保留</span></p>${button('supplement','继续补充要求')}${button('execute',d.status==='failed'?'重试执行':'确认执行',!plan.valid||d.status==='parsing'||d.input.trim()?'disabled':'','primary')}</div>`;
 ...
 render(focusId){
  ...
  this.el.dataset.view=this.view||this.mode;
  const motionBefore=capturePanel(this.el);
  this.el.classList.toggle('assistant-expanded',Boolean(d.clarification||plan.changes.length||result||this.mode==='advanced'||['prompt','help'].includes(this.view)));
  ...
  const contentChanged=Boolean(this.lastPanelKey&&panelKey!==this.lastPanelKey);
  ...
  this.el.innerHTML=`<div class="assistant-shell"><div id="assistant-live" class="sr-only" role="status" aria-live="polite"></div><header class="assistant-header"><div class="assistant-headline"><h2 id="chat-title" tabindex="-1"><span class="assistant-title-mark" aria-hidden="true"><svg viewBox="0 0 24 24"><path d="M4 10v4M8 6v12M12 3v18M16 7v10M20 10v4"/></svg></span>旁白助手</h2><span class="assistant-preserve-note">原片与原声保留</span></div><div class="assistant-context-row"><label class="assistant-scene-label" for="assistant-scene">修改场景</label><select id="assistant-scene" title="${esc(sceneLabel(this.origin.film,this.context.sceneId))}" ${locked?'disabled':''}>${this.origin.film.scenes.map(s=>`<option value="${esc(s.id)}" ${s.id===this.context.sceneId?'selected':''}>${esc(sceneLabel(this.origin.film,s.id))}</option>`).join('')}</select><div role="tablist" aria-label="旁白助手模式" class="assistant-tabs"><span class="assistant-tab-indicator" aria-hidden="true"></span>${['ordinary','advanced'].map(m=>`<button role="tab" id="assistant-tab-${m}" aria-controls="assistant-panel" aria-selected="${this.mode===m}" tabindex="${this.mode===m?0:-1}" data-assistant="mode" data-mode="${m}" ${locked?'disabled':''}>${m==='ordinary'?'普通模式':'高级编辑'}</button>`).join('')}</div></div></header><div class="assistant-scroll" id="assistant-panel" role="tabpanel" aria-labelledby="assistant-tab-${this.mode}">${this.voiceStatusHTML()}${content}${d.status==='failed'?`<div class="assistant-notice" id="assistant-failure" tabindex="-1"><strong>本次未完成</strong><p>${esc(d.error)}</p></div>`:''}</div><footer class="assistant-footer">${this.footerHTML(plan,result,allAccepted)}${d.status==='failed'?button('cancel-adjustment','取消调整','','block'):''}</footer></div>`;
  ...
  if(this.visible&&globalThis.innerWidth<=1100&&(focusId||contentChanged)&&this.el.querySelector('.assistant-review'))requestAnimationFrame(()=>{if(this.visible&&this.el.isConnected&&this.el.querySelector('.assistant-review'))this.el.scrollIntoView({block:'start',behavior:'instant'});});
  if(this.visible)animatePanel(this.el,motionBefore,{content:contentChanged});
 }
 ...
 change(e){const el=e.target;if(this.busy)return;
  try{
   ...
   else if(el.dataset.field){const f=FIELD_MAP[el.dataset.field];editDraft(this.draft,[{field:f.key,value:JSON.parse(el.value),...(f.type==='set'?{op:el.checked?'add':'remove'}:{}),...(f.group==='people'&&(el.dataset.character||this.person)?{targetCharacterId:el.dataset.character||this.person}:{})}],this.context);}
   else if(el.id!=='assistant-alias')return;
   this.hooks.services?.syncS1VoiceDraft?.(this);this.parseId++;this.hooks.save();this.render(el.id||undefined);if(el.id==='assistant-scope'&&el.value==='specified')this.el.querySelector('.assistant-targets input')?.focus();
 ...
 async generate(text=this.draft.input){
  ...
  try{
   ...
   applyInterpretation(d,result,context);if(result.semantic)d.semantic=result.semantic;const plan=this.plan,ready=plan.valid&&result.kind!=='explain';
   const message=ready?this.confirmationText(plan):result.kind==='patch'?(plan.errors.join(' ')||d.message||'请核对修改方案。'):result.message||d.message||plan.errors.join(' ');
 ...
 async execute(){
  ...
  finally{this.busy=false;this.draft=d;this.context=context;if(!succeeded)this.view='card';this.render(succeeded?'assistant-result-title':'assistant-failure');this.notify(succeeded?this.resultText():errorMessage,succeeded?'result':'error');}
 }
 ...
 async action(action,b){
  ...
  if(action==='cancel-adjustment'){
   if(this.draft.status!=='failed')return;
   this.hooks.stop();this.parseId++;this.draft=openDraft(this.session,this.context,{fresh:true});this.mode='ordinary';this.category=null;this.view=null;
   this.hooks.onCancel?.(this.context);this.hooks.save();this.render('assistant-input');return this.notify('已取消本次调整，原有配音和已采用的版本保留。');
  }
  ...
  if(action==='speak'||action==='speak-result'){this.hooks.stop();this.hooks.say('assistant-summary',action==='speak'?spokenChanges(this.plan,{full:true,characters:this.context.registry,real:!!this.hooks.services,video:this.videoVersions}):this.resultText(),true);return;}
  if(action==='available-version'){
   const result=this.hooks.services?.recover?.(this);if(!result)return;
   applyInterpretation(this.draft,result,this.context);this.draft.input='';this.draft.history.push({who:'assistant',text:result.notes.join(' ')});this.view='card';this.hooks.save();this.render('assistant-command-title');return this.notify(this.confirmationText(),'ready');
  }
 ...
 continueFlow(){
  if(!isAcceptedResult(this.session,this.draft))return;
  const candidates=this.draft.result.candidates,context=this.context;
  this.hooks.stop();this.draft=openDraft(this.session,context);this.pinnedScene=false;this.mode='ordinary';this.category=null;this.view=null;this.person='';
  this.hooks.save();this.render();this.hooks.focusVideo?.();return this.hooks.onAccepted(context,this.session,candidates);
 }
}
