...
自动播报限于位置、关键状态、简短修改摘要和澄清问题；Tab 读名称与必要状态；完整摘要可主动重听。长技术提示词、历史、全部标签、逐字输入和重复进度不自动朗读。录音期间暂停视频、旁白、引导和焦点语音；读屏优先时关闭站内语音并保留 ARIA 状态。侧栏「哪些内容会播报」和操作帮助提供同一套说明。自动引导、Tab 播报和人物介绍仍保留各自的语速设置。
