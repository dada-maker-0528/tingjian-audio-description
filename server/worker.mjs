...
const ranges=new Set(Object.values(film.samples).map(x=>`${x.start}:${x.duration}`));
...
export function validateInput(body){
 ...
 if(body.kind!=='narration'||body.filmId!==film.id||!ranges.has(`${body.start}:${body.duration}`)||!['normal','slow'].includes(body.speed)||!['balanced','concise'].includes(body.density))throw new SpeechError('bad_input','这组旁白设置与当前视频不匹配，请刷新后再试。',400);
...
export async function handleTTS(request,env,connector=openSpeech){
 ...
 try{
  ...
  for(const cue of source){let text=cue.text,pcm=await connection.synthesize(text,body.speed);const gap=cue.maxDuration;
  ...
  return json({provider:'volcengine',filmId:film.id,sampleRate:24000,trackDuration:film.duration,segments});
...
export default {async fetch(request,env,ctx){
 ...
 if(url.pathname==='/api/media/'+film.fileName)return serveDemoVideo(request,env.ASSETS,'/'+film.video);
 if(url.pathname==='/api/tts/status')return json({configured:!!env.VOLC_TTS_KEY,provider:'volcengine',label:'豆包语音',model:'seed-tts-2.0',filmId:film.id});
