...
const keyboardReader=new FocusReader({enabled:()=>prefs.focusReadout!==false&&!prefs.reader&&!voiceCapturing(),beforeSpeak:()=>{stopGuide();pauseMedia();},status:$('#focus-readout'),rate:()=>prefs.promptRates.focus});
keyboardReader.start();
const stageNames={roles:'认识角色',short:'第 1 个场景',medium:'连续场景确认',full:'完整制作',complete:'制作完成','full-review':'完整视频复看'};
const stageTitle=owner=>sceneStageLabel(owner)||stageNames[owner?.stage]||'准备素材';
...
function stopAll(){assistant?.cancelVoice();keyboardReader.stop();stopGuide();pauseMedia();}
function updateGuideUI(){
 ...
 const notice=document.querySelector('.demo-notice>span:nth-of-type(2)');if(notice)notice.textContent=speech.available?(liveSpeech()?'演示模式：画面分析与修改执行为演示；语音合成已接入豆包':'演示模式：画面分析与修改执行为演示；当前使用本地演示音频'):'演示模式：画面分析与修改执行为预设演示';
}
...
function renderRoles(){route='roles';page(shell(`<div class="stage-header"><div class="eyebrow">02 / 认识角色</div><h1>先认识这段视频中的人物</h1><p>已按原片顺序整理 ${film.scenes.length} 个场景，先认识其中的 ${roles.length} 位主要人物。</p></div>${rolesHTML()}<div class="action-bar"><span class="hint" id="role-hint">${icon('volume')}${prefs.shortcuts?'数字 1—'+Math.min(roles.length,9)+' 再听人物 · ':''}空格继续</span><div class="actions"><button class="btn" data-action="roles-all">${icon('replay')}再听全部</button><button class="btn" data-action="chat">修改角色介绍</button><button class="btn primary" data-action="confirm">继续到首个场景 <kbd>空格</kbd> ${icon('arrow')}</button></div></div>`,1),'roles',true,{guide:false});enterRoles();}
const roleReviewOpen=()=>modal.open&&modal.dataset.kind==='roles';
function enterRoles(){if(prefs.guide&&!prefs.reader)return playRoles();announce(roleIntroduction(roles,{sceneCount:film.scenes.length})+roleChoices(roles,{shortcuts:prefs.shortcuts,completed:false}));}
...
function sceneOverview(count){
 ...
 return `<section class="scene-scope" aria-label="本次场景范围"><p><strong>本次试听：${sceneScopeLabel(film,count)}</strong><span>本版已确认 ${confirmed} / ${film.scenes.length} 个场景</span></p><p>${range.scenes.map((s,i)=>`${i+1}. ${esc(s.title)}`).join(' → ')}</p><details><summary>查看完整场景顺序（共 ${film.scenes.length} 个）</summary><ol class="scene-order">${film.scenes.map((s,i)=>`<li class="${i<count?'included':''}"><strong>场景 ${i+1} · ${esc(s.title)}</strong><span>${time(s.end-s.start)} · ${i<confirmed?'已确认':i<count?'本次试听':'待扩展'}</span></li>`).join('')}</ol></details></section>`;
}
function renderSample({introPrefix='',regenerating=false}={}){
 const stage=task.stage;route=stage;const first=stage==='short',count=scenePreviewCount(task),range=sceneRange(film,count);task.sceneCount=count;
 const scope=sceneScopeLabel(film,count),target=Math.min(3,film.scenes.length);
 const heading=first?'先确认第一个完整场景':`确认${scope}的连续效果`;
 const sub=first?`从“${film.scenes[0].title}”开始，听完整个场景再决定是否扩展。`:'按原片顺序连续播放，确认叙述、音色与场景衔接是否合适。';
 const primary=first&&film.scenes.length>1?`满意，生成前 ${target} 个场景`:'这些场景满意，生成整片';
 ...
 const entrance=page(shell(`<div class="stage-header"><div class="eyebrow">${first?'03 / 首个场景':'04 / 连续场景'}</div><h1>${heading}</h1><p>${esc(sub)}</p></div>${sceneOverview(count)}<div class="sample-layout"><div>${playerHTML(scope+' · 口述版')}<div class="sample-context">${icon('info')}覆盖 ${count} 个完整场景 · 保持原片顺序 · 播放结束后由你决定下一步</div><details class="transcript"><summary>${icon('chevron')}查看本次场景的旁白</summary><div id="transcript-text"></div></details></div>${settingsHTML(task.candidate,task.version)}</div><div class="action-bar"><span class="hint"><kbd>空格</kbd> 播放 / 暂停 · <kbd>Enter</kbd> 确认当前场景</span><div class="actions"><button class="btn" data-action="replay">${icon('replay')}再听本次场景</button>${extend}<button class="btn primary" data-action="confirm" data-scene-confirm id="confirm-sample" disabled title="等待场景准备完成">${primary} ${icon('arrow')}</button></div></div>`,stageIndex(stage)),stage,true,{introPrefix:prefix});
 ...
 return settleRegeneration(ready,entrance,{current:()=>task===owner&&task.version===version&&task.sceneCount===count&&route===stage,version,regenerating,full:false});
}
...
async function initPlayer(start,duration,settings,position){
 ...
 narration.src=src;if(label)label.textContent=`${playbackScope()} · ${currentAudioSource==='volcengine'?'豆包 · '+voiceInfo(settings.voice).name:'本地默认旁白'}`;
 ...
 try{await Promise.all([mediaReady(v),mediaReady(narration)]);if(generation!==mediaGeneration)return false;v.currentTime=Math.max(start,Math.min(clipEnd,position||start));narration.currentTime=v.currentTime;playerReady=true;main.querySelectorAll('[data-scene-confirm]').forEach(b=>{b.disabled=false;b.removeAttribute('title');});updateProgress();announce((playbackScope()+'旁白')+'已准备好，'+(currentAudioSource==='volcengine'?'由豆包语音合成。':'使用本地演示音频。'));return true;}catch(e){if(generation===mediaGeneration)mediaError(e);return false;}
}
...
function syncFrame(){if(!playing||!player)return;if(player.currentTime>=clipEnd-.015){player.currentTime=Math.max(clipStart,clipEnd-.001);pauseMedia();updateProgress();return;}if(narrationOn&&Math.abs(narration.currentTime-player.currentTime)>.20)if(narration.src)narration.currentTime=player.currentTime;updateProgress();raf=requestAnimationFrame(syncFrame);}
function updateProgress(){if(!player)return;const at=player.currentTime;const elapsed=Math.max(0,Math.min(clipEnd-clipStart,at-clipStart));if($('#seek')){$('#seek').value=elapsed;$('#seek').setAttribute('aria-valuetext',durationWords(elapsed)+'，共'+durationWords(clipEnd-clipStart));}$('#player-time')&&($('#player-time').textContent=`${time(elapsed)} / ${time(clipEnd-clipStart)}`);
 ...
 const sceneLabel=$('#scene-playback');if(sceneLabel){sceneLabel.hidden=sceneIndex<0;const text=sceneIndex>=0?`场景 ${sceneIndex+1} / ${film.scenes.length} · ${film.scenes[sceneIndex].title}`:'';if(sceneLabel.textContent!==text)sceneLabel.textContent=text;}
...
function showHelp(){showModal('用你习惯的方式操作',`<p class="dialog-copy">所有操作都可以用键盘完成。人物介绍页按空格继续、按数字键再听人物。输入框内保留正常输入。</p><table class="help-table"><tbody><tr><td><kbd>空格</kbd></td><td>首页创建 · 人物页继续 · 试听页播放或暂停</td></tr><tr><td><kbd>1—9</kbd></td><td>按当前编号播放视频、再听角色</td></tr><tr><td><kbd>Tab</kbd></td><td>移动到下一个操作，并朗读按钮名称</td></tr><tr><td><kbd>Shift + Tab</kbd></td><td>返回上一个操作，并朗读名称</td></tr><tr><td><kbd>O</kbd></td><td>非输入区域开始语音输入；录音中再按 O 结束识别</td></tr><tr><td><kbd>Enter</kbd></td><td>输入框发送文字；录音中结束识别并发送最终文字</td></tr><tr><td><kbd>Shift + Enter</kbd></td><td>输入框内换行</td></tr><tr><td><kbd>Esc</kbd></td><td>停止录音或播报；关闭提示词预览；助手保持展开</td></tr></tbody></table><p class="inline-note">中文语音输入由浏览器识别，首次需允许麦克风。录音期间暂停影片和播报；发送文字不会直接执行修改。侧栏底部“哪些内容会播报”可查看完整规则。</p><div class="dialog-actions"><button class="btn" data-action="reset-confirm">重置演示</button><button class="btn primary" data-action="close-modal">知道了</button></div>`);}
...
function resume(){closeModal();stopAll();runId++;if(!task)return renderUpload();selectFilm(task.assetId);if(task.completed)return renderComplete();if(task.stage==='roles')renderRoles();else if(['short','medium'].includes(task.stage))renderSample();else if(task.stage==='full'){if(canComplete(task)){processing('full','继续准备完整视频',['加载已确认设置','检查完整视频','准备成片'],()=>{task.completed=true;task.stage='complete';persist();renderComplete();},4);}else{task.stage='medium';task.sceneCount=Math.min(3,task.totalScenes);renderSample();}}}
function editTarget(){
...
function mountAssistant(){
 ...
 const inner=container.querySelector('.sample-layout');if(inner){const settings=inner.querySelector(':scope > .setting-card');if(settings){const details=document.createElement('details');details.className='current-audio-settings';const summary=document.createElement('summary');summary.textContent='当前配音设置';details.append(summary,settings);inner.firstElementChild.append(details);}inner.classList.add('preview-column');}
 const layout=document.createElement('div');layout.className='assistant-workspace';const workspace=document.createElement('div');workspace.className='assistant-main-column';while(container.firstChild)workspace.append(container.firstChild);const mount=document.createElement('div');mount.className='assistant-mount';mount.append(chat);layout.append(workspace,mount);container.append(layout);container.classList.add('assistant-container');
