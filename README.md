...
新设备默认开启产品语音引导；已保存的关闭或读屏优先设置继续生效。进入首页、上传、整理素材、人物介绍、短片试听、长片试听、复验、制作完成和完整观看页面时，先说明当前位置、当前步骤和可用操作。准备页面会等开场引导完成后再进入下一步；首次访问受浏览器自动播放限制时，可点击“再听提示”开始。

人物页先播报第二步和人数，再依次介绍第一位、第二位等人物，最后主动询问继续还是再听。数字键 1—9 对应当前人物，空格直接进入 7 秒试听，点击“继续到 7 秒试听”也可继续。单独再听后会再次说明下一步；切页、停止提示或选择其他人物会取消旧序列。人物回看弹窗中的空格只关闭介绍，不会推进制作阶段。
...
完成页和完整观看页均提供“修改并重新生成”。完整视频使用独立修改草稿，不覆盖其他制作草稿；可反复修改、播放检查，满意后另存新版。原版和已保存版本保留，未完成或失败的生成无法保存为就绪成片。回到首页也可继续未完成的完整视频修改草稿。中样片修改后仍保持短片复验要求。
