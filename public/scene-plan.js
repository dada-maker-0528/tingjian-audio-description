...
export const scenePreviewCount=task=>task?.stage==='short'?1:task?.sceneCount||1;
export const sceneScopeLabel=(film,count)=>count===1?'第 1 个场景':count===film.scenes?.length?`全部 ${count} 个场景`:`前 ${count} 个场景`;
export const sceneStageLabel=task=>task?.stage==='short'?'第 1 个场景':task?.stage==='medium'?`前 ${task.sceneCount||3} 个场景`:'';
