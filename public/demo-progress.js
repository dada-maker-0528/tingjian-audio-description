...
export function demoPlan({kind,film,start=0,duration=7,settings,videoURL,prepareNarration}){
 ...
 if(kind==='analyzing')return [
  ...
  {start:'加载预设镜头与场景',complete:'预设镜头与场景已加载',run:async({log,node})=>{
   ...
   node('presetScenes','按预设镜头定位短片、长片与复验的试听范围。',{short_start:film.samples.short.start,medium_start:film.samples.medium.start,verify_start:film.samples.verify?.start});
   log('试听顺序已装配',{flow:['short','medium','verify_if_edited','full']});
  }},
 ...
 return [
  ...
  {start:kind==='full'?'准备完整影片的旁白版本':'匹配本次样片版本',complete:'对应旁白音轨已准备好',run:async({signal,log,node})=>{
   ...
   node('cache','按音色、范围与旁白版本查找缓存，复用已准备的音轨。');
   node('narration','如果缓存与预制音轨均未命中，就请求在线合成并校验返回数据。');
