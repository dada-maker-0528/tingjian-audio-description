...
export class SpeechService{
 ...
 async guide(text,signal,voice=DEFAULT_VOICE){
  ...
  if(!blob){
   ...
   if(!response?.ok){if(!this.available)throw new Error('豆包语音尚未连接，这段提示还没有缓存。');response=await this.request({kind:'guide',text,voice},signal);}
  ...
  aborted(signal);const url=URL.createObjectURL(blob);this.cache.set(key,url);return url;
 }
