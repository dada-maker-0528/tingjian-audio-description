  ...
  return startJob(p,'pipeline',async({signal,stage})=>{
    ...
    for(let i=0;i<p.scenes.length;i++){
      ...
      if(personal&&p.provenance==='live-ai'&&!s.manualEdited){
        ...
        if(!window&&s.category==='optional'){
          s.omission={reason:'原声持续说话，没有可用的旁白空隙，保留原声。',text:s.text};
          s.category='none';s.text='';changed(p,s,true);await save(p);
          await stage(2,`「${s.title}」保留原声，不插入可省略旁白`);continue;
        }
