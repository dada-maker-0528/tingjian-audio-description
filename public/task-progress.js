...
export function createTaskProgress(root,{steps,mode='live',completeLabel='样片已准备好',onReady=()=>{},onComplete=()=>{},onRetry,timing=TASK_TIMING}={}){
 ...
 function append(e){
  ...
  for(const message of e.lines||[e.message]){
   ...
   line.innerHTML=e.type==='code-batch'?`<code>${codeHTML.get(message)??highlightCode(message)}</code>`:e.type==='code'?`<code>${highlightJSON(message)}</code>`:`<span>${esc(message)}</span>${e.source?`<small>执行逻辑 · ${esc(e.source)}</small>`:''}`;
 ...
 function tick(){
  if(disposed||!root.isConnected||paused)return;
  const event=queue.tick(Date.now());
  waiting.hidden=!!event||queue.pending.length>0||!queue.states.includes('running')||Date.now()-lastEventAt<1200;
  ...
  if(event.type==='finish'){
   ...
   finishTimer=setTimeout(()=>{if(!disposed){q('.task-continue').hidden=false;onReady();}},queue.timing.finishMs);clearInterval(timer);return;
  }
 ...
 motion.onclick=()=>{paused=!paused;motion.textContent=paused?'继续展示':'暂停展示';motion.setAttribute('aria-pressed',String(paused));if(!paused&&scrollTarget)followEnd(scrollTarget);};
 ...
 return {
  ...
  complete(){queue.push({id:'finish',type:'finish'});tick();return done;},
  ...
  dispose(){disposed=true;clearInterval(timer);clearTimeout(finishTimer);cancelAnimationFrame(scrollFrame);controller.abort();resolveDone(false);},
