...
test('opening guidance names Tingjian and its actual library destination',()=>{
 const full=pageGuide('home'),brief=briefPageGuide('home');
 assert.match(full,/听见.*进入我的视频/);
 assert.match(brief,/听见.*进入体验/);
 for(const guide of [full,brief])assert.doesNotMatch(guide,/智享视界/);
});
