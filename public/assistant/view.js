...
export class NarrationAssistant{
 constructor(dialog,hooks){this.el=dialog;this.hooks=hooks;this.mode='ordinary';this.category=null;this.view=null;this.parseId=0;this.busy=false;this.voiceMessage='';this.composing=false;
  ...
  dialog.addEventListener('keydown',e=>{
   ...
   const action=assistantKeyAction({...e,key:e.key,keyCode:e.keyCode,isComposing:e.isComposing||this.composing,repeat:e.repeat,shiftKey:e.shiftKey,ctrlKey:e.ctrlKey,metaKey:e.metaKey,altKey:e.altKey},{enabled:this.hooks.shortcuts(),editable:!!e.target.closest('textarea,input,select,[contenteditable="true"]'),inputFocused:e.target.id==='assistant-input',recording:this.voice.active});
 ...
 confirmationText(plan=this.plan){return spokenChanges(plan,{characters:this.context.registry});}
 ...
 footerHTML(plan,result,allAccepted){
  ...
  if(result)return `${button('speak-result','听结果摘要')}${allAccepted?button('continue-flow',this.origin.canContinue?'满意并继续':'满意并完成','','primary block'):button('accept-all',d.result.candidates.length===1?(this.origin.canContinue?'满意并继续':'满意并完成'):'请逐个确认场景方案',d.result.candidates.length===1?'':'disabled','primary block')}<p class="assistant-meta">确认的是模拟方案；原有配音保留。</p>`;
 ...
 async generate(text=this.draft.input){
  ...
  try{
   const result=await assistantServices.parse(text,context,clone(d));if(id!==this.parseId||this.draft!==d||d.revision!==revision||d.status==='running')return;
 ...
 async execute(){
  ...
  try{const result=await assistantServices.run(run);if(!finishRun(session,d,result))throw new Error(d.error||'结果已过期，当前版本未改变。');this.hooks.save();this.hooks.onResult(context,result);succeeded=true;}
 ...
 async action(action,b){
  ...
  if(action==='speak'||action==='speak-result'){this.hooks.stop();this.hooks.say('assistant-summary',action==='speak'?spokenChanges(this.plan,{full:true,characters:this.context.registry}):this.resultText(),true);return;}
