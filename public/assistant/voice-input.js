...
export class VoiceInput{
 constructor({factory=()=>{const C=recognitionConstructor();return C?new C():null;},beforeStart=()=>{},onState=()=>{},onText=()=>{},onSubmit=()=>{},stopTimeout=5000,maxDuration=90000}={}){
 ...
 start(prefix=''){
  ...
  let recognition;try{recognition=this.factory();}catch{recognition=null;}
  ...
  recognition.onerror=event=>{if(!alive())return;session.failed=true;this.cleanup();this.onText(text(),{final:!session.interim,recording:false});this.setState('error',ERRORS[event.error]||'语音识别失败，文字已保留，可以重新输入。');try{recognition.abort();}catch{}};
  recognition.onend=()=>{
  ...
  try{recognition.start();return true;}catch(error){if(alive()){this.cleanup();this.setState('error',error.name==='NotAllowedError'?ERRORS['not-allowed']:'语音输入未能启动，请重试或直接输入文字。');}return false;}
 }
 finish(send=false){
  ...
  this.stopTimer=setTimeout(()=>{if(this.current!==session)return;this.cleanup();this.setState('error','结束识别超时，未自动发送。文字已保留，请检查后手动发送。');try{session.recognition.abort();}catch{}},this.stopTimeout);
