...
export function sampleRange(p,stage) {
  ...
  if(w.sceneFlow){
    const count=stage==='full'?p.scenes.length:stage==='short'?1:w.sceneCount||Math.min(3,p.scenes.length);
    const scenes=p.scenes.slice(0,count);let end=0;
    for(const s of scenes){if(Math.abs(s.start-end)>.02||s.end<=s.start)throw new Error('场景边界不连续，请重新分析');end=s.end;}
    if(!scenes.length)throw new Error('缺少完整场景');return {start:0,end};
  }
