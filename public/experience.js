...
const backendAssistant=createAssistantBackend({store:assistantStore,save:persist,stop:stopAll,preview:audio=>extraAudio=audio,baseline:async a=>{const key=a.origin.settings.speed+'-'+a.origin.settings.density;return audioBaseline(narration.src||assetURL(film.fallbackAudio[key]),activeNarrationCues||film.narration[key]);}});
...
function showUploadedProgress(initial){
 ...
 const view=createTaskProgress($('#task-progress'),{steps,mode:'live',completeLabel:'已准备好',onReady:()=>{main.querySelector('.processing h1').textContent='已准备好';},onRetry:async()=>{try{const p=await projectRequest(initial.id);showUploadedProgress(await projectRequest(p.id,{action:'retry',requestId:crypto.randomUUID(),revision:p.workflow.revision}));}catch(e){toast(e.message);}}});processingView=view;
 void (async()=>{
  try{let p=initial;
...
function updateGuideUI(){
 ...
 const notice=document.querySelector('.demo-notice>span:nth-of-type(2)');if(notice)notice.textContent='示例影片 · 旁白修改连接真实服务';const voiceSource=document.querySelector('.notice-right');if(voiceSource)voiceSource.textContent=liveSpeech()?'豆包配音':'本地配音';
}
...
function renderUpload(){route='upload';page(shell(`<div class="stage-header"><div class="eyebrow">01 / 选择素材</div><h1>选择视频</h1></div><div class="upload-layout"><div class="upload-box" id="drop-zone">${icon('upload')}<h2>上传视频文件</h2><p>拖到这里，或选择设备上的视频</p><div class="upload-actions"><button class="btn primary" data-action="upload">${icon('plus')}选择视频文件</button><button class="btn" data-action="link">${icon('link')}粘贴链接</button></div></div><div class="demo-source"><span class="label">示例视频</span>${films.length>1?`<label class="field-label" for="catalog-source">选择公开视频</label><select class="text-input" id="catalog-source">${films.filter(hasNarration).map(f=>`<option value="${f.id}" ${f.id===film.id?'selected':''}>${esc(f.title)} · ${time(f.duration)}</option>`).join('')}</select>`:''}<div class="source-top"><img src="${mediaAsset(film.covers[0].file)}" alt="青年骑车带着女孩"><div><h3>${esc(film.title)}</h3><p>${durationWords(film.duration)} · 中文原声 · ${roles.length} 位角色 · ${film.scenes.length} 个场景</p></div></div><button class="btn block" data-action="demo">使用演示视频 ${icon('arrow')}</button></div></div><p class="inline-note">${icon('info')}上传后自动分析人物与场景，也可使用示例视频。</p><div id="local-preview"></div>`,0),'upload');const dz=$('#drop-zone');dz.addEventListener('dragover',e=>{e.preventDefault();dz.style.borderColor='#111';});dz.addEventListener('dragleave',()=>dz.style.borderColor='');dz.addEventListener('drop',e=>{e.preventDefault();if(e.dataTransfer.files[0])handleFile(e.dataTransfer.files[0]);});if($('#catalog-source'))$('#catalog-source').addEventListener('change',e=>{stopGuide();selectFilm(e.target.value);const box=$('.demo-source .source-top');box.querySelector('img').src=mediaAsset(film.covers[0].file);box.querySelector('img').alt=film.covers[0].alt;box.querySelector('h3').textContent=film.title;box.querySelector('p').textContent=durationWords(film.duration)+' · 中文原声 · '+roles.length+' 位角色 · '+film.scenes.length+' 个场景';});}
...
function sceneOverview(count){
 ...
 return `<section class="scene-scope" aria-label="本次场景范围"><div class="scene-scope-heading"><h2>${count===1?'第 1 个场景 · '+esc(range.scenes[0].title):sceneScopeLabel(film,count)}</h2><span>${time(range.duration)}</span></div>${count>1?`<p class="scene-route">${range.scenes.map((s,i)=>`${i+1}. ${esc(s.title)}`).join(' / ')}</p>`:''}<details><summary>场景列表 · 已确认 ${confirmed}/${film.scenes.length}</summary><ol class="scene-order">${film.scenes.map((s,i)=>`<li class="${i<count?'included':''}"><strong>${i+1}. ${esc(s.title)}</strong><span>${time(s.end-s.start)} · ${i<confirmed?'已确认':i<count?'本次试听':'待制作'}</span></li>`).join('')}</ol></details></section>`;
}
...
function freshWatch(){return {id:task.id,assetId:film.id,title:task.title,settings:{...task.confirmed},sourceProjectId:task.sourceProjectId,position:0};}
...
async function initPlayer(start,duration,settings,position=0,candidate=null){
 ...
 v.src=film.sourceUrl||((window.__TINGJIAN_ASSETS__||location.protocol==='file:')?assetURL(film.video):'/api/media/'+film.fileName);v.playbackRate=1;narration.playbackRate=1;narration.preload='auto';narration.volume=settings.gain;
 ...
 if(liveSpeech()&&!candidate&&!film.projectId){
 ...
 if(!candidate){
  ...
  try{const current=await backendAssistant.current(owner?.id);if(current?.originalNarrationUrl){src=current.originalNarrationUrl;activeNarrationCues=current.film.scenes.flatMap(s=>s.cues);currentAudioSource='revision';}}
  catch(error){mediaError(error);return false;}
 }
 ...
 try{await Promise.all([mediaReady(v),mediaReady(narration)]);if(generation!==mediaGeneration)return false;v.currentTime=Math.max(start,Math.min(clipEnd,position||start));narration.currentTime=Math.max(0,v.currentTime-audioOffset);playerReady=true;main.querySelectorAll('[data-scene-confirm]').forEach(b=>{b.disabled=false;b.removeAttribute('title');});updateProgress();announce((playbackScope()+'旁白')+'已准备好，'+(currentAudioSource==='revision'?'使用实际生成的新版配音。':film.projectId?'使用已生成的配音。':currentAudioSource==='volcengine'?'由豆包语音合成。':'使用本地演示音频。'));return true;}catch(e){if(generation===mediaGeneration)mediaError(e);return false;}
}
...
async function playMedia(){
 ...
 try{await v.play();if(narrationOn){narration.currentTime=Math.max(0,v.currentTime-audioOffset);await narration.play();}if(generation!==mediaGeneration){v.pause();return;}raf=requestAnimationFrame(syncFrame);}catch{mediaError(new Error('播放未开始，请再次点击播放。'));}
}
...
const assistant=new NarrationAssistant(chat,{
 ...
 async onAccepted(context,session,candidates){
  ...
  const duration=['short','medium'].includes(route)?sceneRange(film,scenePreviewCount(task)).duration:film.duration;
...
function mountAssistant(){
 ...
 const meta=document.createElement('span');meta.className='workbench-meta';meta.textContent=time(film.duration)+' · '+film.scenes.length+' 个场景';
 ...
 const inner=container.querySelector('.sample-layout');if(inner){const settings=inner.querySelector(':scope > .setting-card');if(settings){const details=document.createElement('details');details.className='current-audio-settings';const summary=document.createElement('summary');summary.textContent='配音设置';details.append(summary,settings);inner.firstElementChild.append(details);}inner.classList.add('preview-column');}
 ...
 workspace.querySelectorAll('.transcript').forEach(el=>{el.open=true;const summary=el.querySelector('summary');if(summary)summary.innerHTML=icon('chevron')+'口述旁白';});
