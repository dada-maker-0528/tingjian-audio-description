...
默认采用精简播报：进入页面只报位置，同一页面重新渲染不重复介绍；模式、分类、场景下拉切换和处理中状态不再追加语音。修改就绪只说「修改已整理好，请确认」，结果就绪只给一句提醒；必要的澄清和错误保留短提示。不会自动念修改清单、全部快捷键或整段转写文字。Tab 读名称与必要状态，分类入口只读类别名及修改数量。
