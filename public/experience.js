...
import {defaults,newTask,confirmStage,canComplete,proposeFeedback,applyFeedback,changeNarrationVoice,copySettings,newFullRevision,markFullRevisionReady,acceptFullRevision} from './flow.js';
...
const stageNames={roles:'认识角色',short:'7 秒样片',medium:'45 秒样片',verify:'7 秒复验',full:'完整制作',complete:'制作完成','full-review':'完整视频复看'};
...
function updateGuideUI(){
 ...
 const notice=document.querySelector('.demo-notice>span:nth-of-type(2)');if(notice)notice.textContent=speech.available?(liveSpeech()?'演示模式：画面分析和语音识别为预设；语音合成已接入豆包':'演示模式：画面分析和语音识别为预设；当前使用本地演示音频'):'演示模式：AI 分析、生成与识别结果为预设演示';
}
async function say(key,text,force=false,{sequence=false}={}){
 const words=promptText(text||currentGuideText(key));if(!words)return false;
 if((!prefs.guide||prefs.reader)&&!force){announce(words);return false;}
...
function page(html,guideKey,focus=true,options={}){stopAll();clearPlayer();main.innerHTML=html;const heading=main.querySelector('h1');if(heading)heading.tabIndex=-1;fillIcons();$('.main-nav [data-action="home"]').classList.toggle('active',route==='home');$('.main-nav [data-action="library"]').classList.toggle('active',route==='library');currentGuide=guideKey||route;if(focus)focusTitle();updateGuideUI();const words=(options.introPrefix||'')+currentGuideText();announce(words);return options.guide===false?Promise.resolve(false):say(currentGuide,words);}
...
function renderHome(query=''){
 ...
 page(`<div class="container"><div class="page-heading"><div><div class="eyebrow">YOUR STORIES, WITH SOUND</div><div class="title-line"><h1>${all?'我的全部视频':'我的视频'}</h1><span class="count">${library.length} 部</span></div><p>选一部，听见故事。也可以创造新的体验。</p></div><button class="btn primary" data-action="create">${icon('plus')}创建新视频<kbd>空格</kbd></button></div>${!all?homeIntro():`<label class="search-box">${icon('search')}<input type="search" id="search" placeholder="搜索视频名称" aria-label="搜索我的视频" value="${esc(query)}"></label>`}${task&&!task.completed?`<div class="draft-strip">${icon('file')}<span>上次制作停在「${stageNames[task.stage]||'认识角色'}」</span><button class="text-btn" data-action="resume">继续上次制作 ${icon('arrow')}</button></div>`:''}${revision&&!revision.completed?`<div class="draft-strip">${icon('file')}<span>完整视频修改草稿：${esc(revision.title)}</span><button class="text-btn" data-action="resume-revision">继续修改完整视频 ${icon('arrow')}</button></div>`:''}<div class="section-heading"><h2>${all?'全部作品':'最近添加'}</h2>${!all&&library.length>6?`<button class="text-btn" data-action="library">查看全部 ${icon('arrow')}</button>`:`<span>公开片库 ${films.length} 部 · 口述版本保存在本机</span>`}</div><div class="library-grid" id="cards">${items.length?items.map((x,i)=>filmCard(x,i,!all&&items.length===1)).join(''):(all?'<div class="empty-state"><h3>没有找到相关视频</h3><p>换个名称再试试。</p></div>':'<div class="empty-state"><h3>还没有保存视频</h3><p>按空格开始制作第一个视频。</p></div>')}${!all?createTile():''}</div>${!all?`<div class="workflow-note"><div><h3>一点点确认，安心听完整片</h3><p>每一步，都有语音陪伴。</p></div><div class="mini-steps"><div class="mini-step"><span class="step-num">01</span><div><strong>先认识故事里的人</strong><small>听角色介绍，熟悉人物</small></div></div>${icon('chevron')}<div class="mini-step"><span class="step-num">02</span><div><strong>先听 7 秒，再听 45 秒</strong><small>告诉 AI 你的感受，随时调整</small></div></div>${icon('chevron')}<div class="mini-step"><span class="step-num">03</span><div><strong>保存，随时再听</strong><small>喜欢的故事，留在你的视频库</small></div></div></div></div>`:''}</div>`,all?'library':'home',false);
 if(all)$('#search').addEventListener('input',e=>{const q=e.target.value;$('#cards').innerHTML=visibleLibrary(library,{query:q,all:true}).map((x,i)=>filmCard(x,i,false)).join('')||'<div class="empty-state"><h3>没有找到相关视频</h3><p>换个名称再试试。</p></div>';});
}
...
function renderUpload(){route='upload';page(shell(`<div class="stage-header"><div class="eyebrow">01 / 选择素材</div><h1>先选择你想听的视频</h1><p>从一段视频开始，我们会先介绍故事里的人。</p></div><div class="upload-layout"><div class="upload-box" id="drop-zone">${icon('upload')}<h2>上传视频文件</h2><p>拖到这里，或选择设备上的视频</p><div class="upload-actions"><button class="btn primary" data-action="upload">${icon('plus')}选择视频文件</button><button class="btn" data-action="link">${icon('link')}粘贴链接</button></div></div><div class="demo-source"><span class="label">第一次来？从演示短片开始</span>${films.length>1?`<label class="field-label" for="catalog-source">选择公开视频</label><select class="text-input" id="catalog-source">${films.filter(hasNarration).map(f=>`<option value="${f.id}" ${f.id===film.id?'selected':''}>${esc(f.title)} · ${time(f.duration)}</option>`).join('')}</select>`:''}<div class="source-top"><img src="${assetURL('assets/'+film.covers[0].file)}" alt="青年骑车带着女孩"><div><h3>${esc(film.title)}</h3><p>${durationWords(film.duration)} · 中文原声 · ${roles.length} 位角色</p></div></div><p>已准备好对应的人物介绍和口述旁白，可完整体验试听、调整和保存。</p><button class="btn block" data-action="demo">使用演示视频 ${icon('arrow')}</button></div></div><p class="inline-note">${icon('info')}本地文件可选择和预览；预设 AI 分析仅适用于内置演示短片。</p><div id="local-preview"></div>`,0),'upload');const dz=$('#drop-zone');dz.addEventListener('dragover',e=>{e.preventDefault();dz.style.borderColor='#111';});dz.addEventListener('dragleave',()=>dz.style.borderColor='');dz.addEventListener('drop',e=>{e.preventDefault();if(e.dataTransfer.files[0])handleFile(e.dataTransfer.files[0]);});if($('#catalog-source'))$('#catalog-source').addEventListener('change',e=>{stopGuide();selectFilm(e.target.value);const box=$('.demo-source .source-top');box.querySelector('img').src=assetURL('assets/'+film.covers[0].file);box.querySelector('img').alt=film.covers[0].alt;box.querySelector('h3').textContent=film.title;box.querySelector('p').textContent=durationWords(film.duration)+' · 中文原声 · '+roles.length+' 位角色';});}
...
function startDemo(){closeModal();task=newTask(film);task.candidate.voice=liveSpeech()?prefs.voice:DEFAULT_VOICE;persist();processing('analyzing','正在认识这个故事',['读取视频','整理出场人物','划分镜头与场景','整理旁白的插入位置'],()=>{renderRoles();},0);}
...
function renderRoles(){route='roles';page(shell(`<div class="stage-header"><div class="eyebrow">02 / 认识角色</div><h1>先认识这段视频中的人物</h1><p>先介绍片中的 ${roles.length} 位主要人物。先熟悉他们，听起来就更容易。</p></div>${rolesHTML()}<div class="action-bar"><span class="hint" id="role-hint">${icon('volume')}${prefs.shortcuts?'数字 1—'+Math.min(roles.length,9)+' 再听人物 · ':''}空格继续</span><div class="actions"><button class="btn" data-action="roles-all">${icon('replay')}再听全部</button><button class="btn" data-action="chat">修改角色介绍</button><button class="btn primary" data-action="confirm">继续到 7 秒试听 <kbd>空格</kbd> ${icon('arrow')}</button></div></div>`,1),'roles',true,{guide:false});enterRoles();}
const roleReviewOpen=()=>modal.open&&modal.dataset.kind==='roles';
function enterRoles(){if(prefs.guide&&!prefs.reader)return playRoles();announce(roleIntroduction(roles)+roleChoices(roles,{shortcuts:prefs.shortcuts,completed:false}));}
...
function settingsHTML(settings,version,owner=task){return `<aside class="setting-card"><h3>这次的旁白设置</h3><div class="version">${owner?.stage==='full-review'?'完整视频版本':'样片版本'} ${version} · ${owner?.confirmed?.version===version?'已确认设置':'等待你试听确认'}</div><dl class="settings-summary"><div class="setting-item"><dt>旁白音色</dt><dd class="current-voice-name">${voiceInfo(settings.voice).label}</dd></div><div class="setting-item"><dt>旁白语速</dt><dd>${settings.speed==='slow'?'稍慢，更从容':'自然语速'}</dd></div><div class="setting-item"><dt>旁白音量</dt><dd>${settings.gain===1?'更清晰':'标准'}</dd></div><div class="setting-item"><dt>描述重点</dt><dd>${settings.density==='concise'?'关键画面，少说一点':'人物与关键动作'}</dd></div></dl><p class="footnote">设置只用于本次视频。<br>原片对白、音乐和播放速度保持不变。</p><button class="btn small" data-action="settings">${icon('volume')}切换音色</button><button class="btn small" data-action="chat">${icon('chat')}${owner?.stage==='full-review'?'修改并重新生成':'和 AI 说说问题'}</button></aside>`;}
...
function renderSample({introPrefix='',regenerating=false}={}){const stage=task.stage;route=stage;const verify=stage==='verify',medium=stage==='medium';const duration=medium?45:7;const heading=verify?'用更新后的设置，再确认 7 秒':medium?'再听 45 秒，看看连续观看是否合适':'先听 7 秒，找到喜欢的旁白';const sub=verify?'换一个镜头，确认刚才的调整在后续内容里也合适。':medium?'沿用你刚才确认的设置，听听连续的故事是否自然。':'听听声音、节奏和描述。想调整的地方，直接告诉 AI。';const primary=medium?(task.mediumEdited?'修改满意，进行短片复验':'满意，制作完整视频'):verify?'复验满意，制作完整视频':'满意，继续制作 45 秒';
 const entrance=page(shell(`<div class="stage-header"><div class="eyebrow">${medium||verify?'04 / 连续试听':'03 / 试听短片'}</div><h1>${heading}</h1><p>${sub}</p></div><div class="sample-layout"><div>${playerHTML(`${duration} 秒${verify?'复验':'样片'} · 口述版`)}<div class="sample-context">${icon('info')}同一部影片 · ${time(film.samples[stage].start)+'—'+time(film.samples[stage].start+duration)} · 播放结束后由你决定下一步</div><details class="transcript"><summary>${icon('chevron')}查看这一段的旁白文本</summary><div id="transcript-text"></div></details></div>${settingsHTML(task.candidate,task.version)}</div><div class="action-bar"><span class="hint"><kbd>空格</kbd> 播放 / 暂停 · <kbd>Enter</kbd> 确认当前样片</span><div class="actions"><button class="btn" data-action="replay">${icon('replay')}再听这段</button><button class="btn primary" data-action="confirm" id="confirm-sample" disabled title="等待视频资源准备完成">${primary} ${icon('arrow')}</button></div></div>`,stageIndex(stage)),verify?'verify':medium?(task.mediumEdited?'medium-edited':'medium'):'short',true,{introPrefix:regenerating?introPrefix+'正在重新生成当前片段，完成后会提示你试听。':introPrefix});
 const owner=task,version=task.version;if(regenerating)showRegeneration(version);const ready=initPlayer(film.samples[stage].start,duration,task.candidate);persist();return settleRegeneration(ready,entrance,{current:()=>task===owner&&task.version===version&&route===stage,version,regenerating,full:false});
}
function showRegeneration(version,full=false){
 ...
 $('.video-wrap').insertAdjacentHTML('beforeend',`<div class="regeneration-overlay" id="regeneration-status" role="status"><span class="regeneration-spinner" aria-hidden="true"></span><strong>${full?'正在重新生成完整视频':'正在重新生成当前片段'}</strong><p>第 ${esc(version)} 版 · 正在应用你的修改</p><p>准备好后会提示你试听。</p></div>`);
...
async function settleRegeneration(ready,entrance,{current,version,regenerating,full}){
 ...
 if(regenerating&&ok){
  const words=`第 ${version} 版${full?'完整视频':'片段'}已准备好。按空格试听，${full?'满意后选择“满意，保存新版”，也可以继续修改。':'满意后继续，也可以再次修改。'}`;
...
async function initPlayer(start,duration,settings,position){
 ...
 if(!liveSpeech()&&(settings.voice||DEFAULT_VOICE)!==DEFAULT_VOICE){mediaError(new Error('所选音色需要在线合成，请开启在线语音或切换默认音色。'));return false;}
 if(liveSpeech()){
  ...
  catch(e){if(signal.aborted||generation!==mediaGeneration)return false;if((settings.voice||DEFAULT_VOICE)!==DEFAULT_VOICE){mediaError(new Error(e.message+' 可以重试或切换默认音色。'));return false;}toast(e.message+' 已使用本地演示音频。');}
 }
...
function updateProgress(){if(!player)return;const at=player.currentTime;const elapsed=Math.max(0,Math.min(clipEnd-clipStart,at-clipStart));if($('#seek')){$('#seek').value=elapsed;$('#seek').setAttribute('aria-valuetext',durationWords(elapsed)+'，共'+durationWords(clipEnd-clipStart));}$('#player-time')&&($('#player-time').textContent=`${time(elapsed)} / ${time(clipEnd-clipStart)}`);
 const settings=playerSettings();const key=settings?`${settings.speed}-${settings.density}`:'normal-balanced';let cue=film?.dialogues?.find(x=>at>=x.start&&at<x.end);if(!cue&&narrationOn)cue=(activeNarrationCues||film?.narration?.[key])?.find(x=>at>=x.start&&at<x.end);if($('#subtitle'))$('#subtitle').innerHTML=captionOn&&cue?`<span>${cue.kind==='narration'?'口述 · ':esc(cue.speaker)+'：'}${esc(cue.text)}</span>`:'';
...
async function applyVoice(){const id=$('#voice-select').value;if(!isVoice(id))return;if(id!==DEFAULT_VOICE&&!liveSpeech()){toast('请先开启在线语音，再应用这个音色。');return;}stopAll();prefs.voice=id;if(route!=='full-review')for(const item of library)if(item.public)item.settings.voice=id;let remake=false;
 if(task&&['roles','short','medium','verify'].includes(route))remake=changeNarrationVoice(task,id);if(route==='full-review'&&revision)remake=changeNarrationVoice(revision,id);
 if(route==='watch'&&watching)watching.settings.voice=id;
 persist();closeModal();
 if(remake&&['short','medium','verify'].includes(route)){await renderSample();toast('已选择'+voiceInfo(id).name+'，请试听新版再确认。');}
...
function resume(){closeModal();stopAll();runId++;if(!task)return renderUpload();selectFilm(task.assetId);if(task.completed)return renderComplete();if(task.stage==='roles')renderRoles();else if(['short','medium','verify'].includes(task.stage))renderSample();else if(task.stage==='full'){if(canComplete(task)){processing('full','继续准备完整视频',['加载已确认设置','检查完整视频','准备成片'],()=>{task.completed=true;task.stage='complete';persist();renderComplete();},4);}else{task.stage='medium';renderSample();}}}
function editTarget(){
 ...
 if(task&&task.assetId===film.id&&['roles','short','medium','verify'].includes(route))return {scope:'sample',source:task,owner:task,settings:copySettings(task.candidate),version:task.version,history:task.chat};
 return null;
}
function openChat(){
 ...
 chat.innerHTML=`<div class="chat-inner"><div class="dialog-heading"><h2 id="chat-title">${target.scope==='full'?'修改完整视频':'和 AI 说说问题'}</h2><button class="icon-btn" data-action="close-chat" aria-label="关闭对话">${icon('close')}</button></div><div class="chat-context">正在修改：${target.scope==='full'?'完整视频':stageNames[task.stage]}<br>当前视频：${esc(film.title)} · 版本 ${target.version}</div><div class="chat-log" id="chat-log"></div><div class="suggestions"><button data-action="feedback" data-text="旁白慢一点">旁白慢一点</button><button data-action="feedback" data-text="旁白大声一点">旁白大声一点</button><button data-action="feedback" data-text="描述少一点">描述少一点</button></div><form class="chat-form" id="chat-form"><label class="sr-only" for="feedback-text">你的修改意见</label><textarea id="feedback-text" placeholder="告诉我，哪里听起来不太合适…"></textarea><div class="dialog-actions"><button type="button" class="btn" data-action="simulate-voice">${icon('mic')}模拟语音输入</button><button type="submit" class="btn primary">发送 ${icon('arrow')}</button></div><p class="chat-note">可以调整旁白语速、音量和描述量。确认修改后，对话会收起并生成新版。语音输入和意见理解仍为预设演示。</p></form></div>`;
...
function feedback(text){
 ...
 const proposal=target.scope==='sample'&&!['short','medium','verify'].includes(target.owner.stage)?{ok:false,message:'当前人物介绍使用预设内容。进入试听后，可以调整旁白的语速、音量和描述量。'}:proposeFeedback(target.settings,text);
...
document.addEventListener('keydown',e=>{
 ...
 if(e.code==='Space'){e.preventDefault();if(route==='home'||route==='library'){if(!prefs.reader){prefs.guide=true;persist();}beginCreate();}else if(route==='upload')$('#file-input').click();else if(['short','medium','verify','watch','full-review'].includes(route)){playing?pauseMedia():playMedia();}}
...
async function boot(){await speech.discover();renderHome();registerTools();}
