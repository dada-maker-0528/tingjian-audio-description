...
在“和 AI 说说问题”中发送意见后，先展示修改摘要，设置尚不生效。选择“确认修改并重新生成”后，右侧对话立即关闭，播放器中央显示生成状态；资源准备成功后才恢复试听和确认操作。取消对话不会应用待确认建议，不支持的意见会保留在对话中说明原因。

完成页和完整观看页均提供“修改并重新生成”。完整视频使用独立修改草稿，不覆盖其他制作草稿；可反复修改、播放检查，满意后另存新版。原版和已保存版本保留，未完成或失败的生成无法保存为就绪成片。回到首页也可继续未完成的完整视频修改草稿。场景试听阶段的修改会重做当前完整场景范围，确认后再扩展。
